...
const validate = (values) => {
    ...
    } else if (values.immatriculation.length < 3) {
    ...
    if (!values.model) {
      ...
      errors.model = "model must be atleast 3 letters";
    } 
    ...
    if (!values.chassis) {
      errors.chassis = "chassis required";
    }
    if (!values. awaitedRevenue) {
      errors. awaitedRevenue = "Expected amount required";
    }
    ...
    if (!values.proprietor) {
      errors.proprietor= "A proprietor  should be assign ";
    }
    return errors;
  };
const EditCars = (props) => {
   ...
   const validate = (values) => {
    ...
    const  initialValues={
      ...
      const  savedValues={  
        immatriculation:props.transfer.imatriculation,
        brand:props.transfer.brand,
        model: props.transfer.model,
        status: props.transfer.status,
        chassis:props.transfer.chassis,
        proprietor:props.transfer.proprietor,
        expectedAmount:props.transfer.expectedAmount,
        location:props.transfer.location
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, onreset) => {
          ...
          setLoading(true)
            fetch(
                `  http://192.168.100.74:4040/user/edit-car/${props.transfer.id}`,
                 {
                   method: "PUT",
                   headers: { 
  ...
  const chassisClasses =
    formik.errors.chassis && formik.touched.chassis
      ...
      formik.errors. awaitedRevenue && formik.touched.awaitedRevenue
        ? `${classes.IFlc} ${classes.invalid}`
        : `${classes.IFlc}`;
    return (
